...
function MarkdownSlide({ content }) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      ...
      elements.push(
        <li key={i} style={{ listStyleType: 'decimal', marginLeft: 20 }}>
    ...
    <div className="commits-slide">
      ...
      <div className="commit-list">
        {recentCommits.map((c, i) => (
          <div key={c.hash || i} className={`commit-item ${i === 0 ? 'latest' : ''}`}>
      ...
      {lastCommit && (
        <div className="latest-commit-detail">
          <Clock size={14} />
          <span>
            Last commit by <strong>{lastCommit.author}</strong> — {lastCommit.date}
          </span>
        </div>
      )}
    </div>
...
export default function SlideRenderer({ slide, gitContext }) {
  ...
  switch (slide.type) {
    case 'cover':
      return (
        <div className="slide-cover">
          {slide.badge && <div className="cover-badge">{slide.badge}</div>}
          <h1>{slide.title}</h1>
          {slide.subtitle && <p className="cover-subtitle">{slide.subtitle}</p>}
          {slide.meta && <p className="cover-meta">{slide.meta}</p>}
          {gitContext && (
    ...
    case 'mermaid':
      return <MermaidView slide={slide} />
    ...
    case 'schema':
      return (
        <div className="slide-schema">
          {slide.description && <p className="slide-desc">{slide.description}</p>}
          <SchemaViewer spec={slide.specContent || gitContext?.openApiSpec} />
        </div>
      )
